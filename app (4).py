...
# Gunakan keras standalone (support Python 3.14, tanpa TensorFlow)
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        st.markdown(
            "<small style='color:#888;'>Skripsi S1 Sains Data<br>UPN Veteran Jawa Timur</small>",
            unsafe_allow_html=True,
        )
    ...
    with col_result:
        ...
        if uploaded_file and run:
            ...
            if model is None:
                st.error("❌ Model gagal dimuat. Pastikan koneksi internet aktif.")
                return
            ...
            if show_gradcam:
                ...
                if heatmap is not None:
                    ...
                    with g1:
                        ...
                        plt.close(fig)
                    with g2:
                        st.image(overlay, caption="Overlay pada Gambar Asli", use_column_width=True)
                    st.caption("Warna merah = area fokus model tertinggi.")
